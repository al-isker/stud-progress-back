...
export class GradeService {
	...
	async createAll(dgmuSubjectListByAllSemesters: DgmuSubjectListWithGradeByAllSemesters, student: Pick<Student, 'id'>) {
		await Promise.all(dgmuSubjectListByAllSemesters.map(async dgmuSubjectListBySemester => {
			await Promise.all(dgmuSubjectListBySemester.subjectList.map(async dgmuSubject => {
	...
	async someUpdate(dgmuSubjectList: DgmuSubjectListWithGrade, student: Pick<Student, 'id' | 'semester'>) {
					...
					data: {
						...
						grade: {
							update: {
								...
								mark: dgmuSubject.mark,
								status: dgmuSubject.status,
								isNew: existingSubject.grade.isNew || (
	...
	async specificUpdate(studentId: number) {
		...
		const { subjectListWithGrade } = await this.dgmuService.findManyOrThrow(student, {
			grade: true 
		})
